<template>
  ...
  &lt;template slot-scope="{ tree, interleaved, expanded, leaf }">
    &lt;div
      ...
      @click="select(tree.text, leaf)"
    >
      &lt;p>&lt;span v-if="!leaf">{{ expanded ? '&#8681;' : '&#8680;' }} | &lt;/span>
        {{ tree.text }}&lt;/p>
    &lt;/div>
  &lt;/template>
          </code>
  ...
  &lt;template slot-scope="{ tree, level, expanded, leaf }">
    ...
    ${ level === 0 ? 'text-white' : '' }
    ${ leaf ? 'hover:bg-indigo-800 hover:text-white' : '' }`"
